...
import { sendScore } from '../ScoreAPI.js';
...
export default class GameOverScene extends Phaser.Scene {
  ...
  async _submitScore() {
    ...
    const sending = this.add.text(CONFIG.WIDTH / 2, this._statusY, '送信中...', {
    ...
    sending.destroy();
    if (ok) {
      this.add.text(CONFIG.WIDTH / 2, this._statusY, '✅ 送信完了！', {
        fontFamily: 'Arial', fontSize: '13px', color: '#4CAF50', stroke: '#000', strokeThickness: 2
      }).setOrigin(0.5);
    } else {
      this.add.text(CONFIG.WIDTH / 2, this._statusY, '⚠️ 送信エラー', {
